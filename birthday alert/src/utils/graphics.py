...
class mainWindow(QDialog) :
	def __init__(self, parent = None) :
		...
		self.checkBox_layout.setMargin(0)
...
class windowFrame(QFrame) :
	...
	def showAll(self) :
		...
		try :
			...
			self.allbdays.showText.append('cannot open ' + globaldata.DB_FILE_PATH)
			quit()
		...
		try :
			...
			self.allbdays.showText.append('cannot read from ' + globaldata.DB_FILE_PATH)
			quit()
		...
		for data in file_data :
			...
			try :
				...
				self.allbdays.showText.append(data[0] + '\t\t' + str(data[1][0]) + '-' + str(data[1][0]))
			except :
				self.allbdays.showText.append('invalid table in ' + globaldata.DB_FILE_PATH)
				quit()
	...
	def addNewAdd(self) :
		...
		if len(self.name) >= 8 :
			writeData += self.name + '\t' + str(self.date) + '-' + str(self.month) + '\n'
		else :
			writeData += self.name + '\t\t' + str(self.date) + '-' + str(self.month) + '\n'

		db_file.write(writeData)
		db_file.close()
